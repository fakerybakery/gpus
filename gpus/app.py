...
class GPUMonitorApp:
    ...
    def register_socketio_events(self):
        ...
        def handle_connect():
            ...
            self.socketio.emit(
                "devices", json.dumps(self.gpu_stats.get_all_devices_info())
            )
            self.socketio.emit(
                "stats", json.dumps(self.gpu_stats.get_all_devices_stats())
            )

            # Send history data for each device
            for i in range(self.gpu_stats.device_count):
                self.socketio.emit(
                    f"history_{i}", json.dumps(self.gpu_stats.get_history(i))
                )
